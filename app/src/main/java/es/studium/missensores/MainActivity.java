...
public class MainActivity extends AppCompatActivity implements SensorEventListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        textViewProximidad = (TextView) findViewById(R.id.sensorProximidad);
        textViewProximidad.setTextSize(30);

        textViewTemperatura = (TextView) findViewById(R.id.sensorTemperatura);
        textViewTemperatura.setTextSize(30);

        textViewLuz = (TextView) findViewById(R.id.sensorLuz);
        textViewLuz.setTextSize(30);

        textViewAceleracion = (TextView) findViewById(R.id.sensorAceleracion);
        textViewAceleracion.setTextSize(30);

        textViewOrientacion = (TextView) findViewById(R.id.sensorOrientacion);
        textViewOrientacion.setTextSize(30);

        sensorManager = (SensorManager) getSystemService(SENSOR_SERVICE);
        if(sensorManager!=null){
            Sensor sensorDeProximidad = sensorManager.getDefaultSensor(Sensor.TYPE_PROXIMITY);
            Sensor sensorDeTemperatura = sensorManager.getDefaultSensor(Sensor.TYPE_AMBIENT_TEMPERATURE);
            Sensor sensorDeLuz = sensorManager.getDefaultSensor(Sensor.TYPE_LIGHT);
            Sensor sensorAcelerometro = sensorManager.getDefaultSensor(Sensor.TYPE_ACCELEROMETER);
            Sensor sensorDeOrientacion = sensorManager.getDefaultSensor(Sensor.TYPE_ORIENTATION);
        ...
        synchronized (this){
            ...
            switch (event.sensor.getType()){
                ...
                case Sensor.TYPE_LIGHT:
                    masData = event.values;
                    textViewLuz.setText(String.format("Luz:%s lumes", masData[0]));
                    break;
                ...
                case Sensor.TYPE_ORIENTATION:
                    ...
                    x = masData[0];
                    y = masData[1];
                    textViewOrientacion.setText(String.format("x: %s\ny: %s", x, y));
                    break;
                default:
                    break;
            }
